...
export default function RunPage() {
  ...
  return (
    <main className="min-h-dvh bg-black text-white p-8 max-w-3xl mx-auto space-y-6">
      ...
      <Link href="/templates" className="text-sm underline opacity-80">
      ...
      <ul className="space-y-2">
        {run.items.map((it) => {
          ...
          const when = it.checkedAt ? new Date(it.checkedAt).toLocaleString() : null;
          return (
            <li
              key={it.id}
              className="flex items-start gap-3 border border-zinc-800 rounded-lg p-3"
            >
              <input
                type="checkbox"
